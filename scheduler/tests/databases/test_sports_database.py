...
#     def get_sports(self, date: datetime) -> List[Dict]:
#         date_str = date.strftime('%Y-%m-%dT%H:%M:%S.%fZ')
#         output: List[Dict] = []
#         if(os.environ.get('STAGE') == 'dev'):
#             results = self.mongo_db['sports_ndb_test_cache'].find_one({'uid': self.uid})['results']
#         else:
#             query_params = {
#                 'filter': {
#                     "and": [
...
#             # self.mongo_db['sports_ndb_test_cache'].update_one({'uid': self.uid}, {'$set': {'uid': self.uid, 'results': results}}, upsert=True)
#         for task in results:
