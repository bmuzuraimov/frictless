<template>
  <v-btn
    ...
    :ripple="false"
    @click="schedule"
    :class="useNotionStore.notionButton.tw_class"
  >
    <img
    ...
    />
    <span class="text-lg fonr-mont">{{ useNotionStore.notionButton.text }}</span>
  </v-btn>
</template>
...
export default {
  ...
  methods: {
    async schedule() {
      ...
      this.useNotionStore.schedule(this.userId)
    }
  }
}
</script>
